...
from core.models import Fly
from leaderboard_client import upload_score, flush_pending
...
class FrogJumpGame:
    ...
    def update(self):
        if self.state == STATE_GAME:
            ...
            elif not self.jumping and not self.charging:
                self.character_img = self.assets.frog_normal
            ...
            if self.jumping:
                ...
                if not self.falling and self.frog_rect.y <= self.target_y:
                    self.frog_rect.y = self.target_y
                    self.falling = True
                if self.falling:
    ...
    def game_over(self):
        ...
        if not self.score_uploaded:
            try:
                upload_score(self.nickname, self.score)
                flush_pending(force=True)
                self.upload_status = "UPLOADED"
            except: self.upload_status = "FAILED"
    ...
    def draw(self):
        if self.state == STATE_NAME_ENTRY:
            ...
            fade.fill((0,0,0)); fade.set_alpha(self.fade_alpha)
        ...
        elif self.state in (STATE_GAME, STATE_GAMEOVER):
    ...
    def draw_gauge(self):
        w, h = 120, 14
        x, y = SCREEN_WIDTH // 2 - w // 2, SCREEN_HEIGHT - 35
        ratio = self.jump_height / MAX_JUMP_HEIGHT
    ...
    def draw_gameover(self):
        ...
        for i, s in enumerate(self.ranking):
            ...
            self.screen.blit(self.font.render(f"{i+1} : {s}", True, color), (SCREEN_WIDTH // 2 - 70, y))
            y += 35
